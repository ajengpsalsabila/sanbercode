describe('OrangeHRM Tests - Login, Forgot Password, Dashboard', () => {
    ...
    it('TC03: Should reset password for valid username', () => {
      ...
      cy.intercept('POST', '**/forgotPassword').as('forgotPassword');
      elements.resetPasswordUsernameField().type(validUsername);
      elements.resetButton().click();
  
      cy.wait('@forgotPassword').its('response.statusCode').should('eq', 200);
      elements.successMessage().should('be.visible');
    });
  
    it('TC04: Should display dashboard components', () => {
      ...
      elements.dashboardMenu().should('be.visible');
      elements.quickLaunchSection().should('exist');
      elements.timeAtWork().should('exist');
    });
